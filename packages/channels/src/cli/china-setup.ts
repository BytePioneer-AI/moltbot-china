...
type ApiLike = {
  ...
  runtime?: {
    config?: {
      writeConfigFile?: WriteConfigLike;
    };
  };
  logger?: LoggerLike;
};
...
type ChannelId = "dingtalk" | "feishu-china" | "wecom" | "wecom-app" | "qqbot";
...
const DEFAULT_PLUGIN_PATH = join(OPENCLAW_HOME, "extensions", "channels");
const LEGACY_PLUGIN_PATH = join(OPENCLAW_HOME, "plugins", "channels");
  ...
  try {
    ...
    while (continueLoop) {
      const selected = await prompter.askSelect<ChannelId | "save" | "cancel">(
        "请选择要配置的渠道",
        [
          { key: "recommended", value: "dingtalk", label: withConfiguredSuffix(next, "dingtalk") },
          { key: "", value: "qqbot", label: withConfiguredSuffix(next, "qqbot") },
          { key: "", value: "wecom", label: withConfiguredSuffix(next, "wecom") },
          { key: "", value: "wecom-app", label: withConfiguredSuffix(next, "wecom-app") },
          { key: "", value: "feishu-china", label: withConfiguredSuffix(next, "feishu-china") },
...
export function registerChinaSetupCli(api: ApiLike): void {
  if (typeof api.registerCli !== "function") {
    return;
  }

  const writeConfig = api.runtime?.config?.writeConfigFile;
  ...
  api.registerCli(
    (ctx) => {
      ...
      root.command("about").description("显示项目信息").action(() => {
        ...
        clackNote(
          "统一渠道插件包，支持 DingTalk、Feishu、WeCom、WeCom App、QQBot。",
          "关于"
        );
