 ...
 * 计算企业微信消息签名
 */
export function computeWecomKfMsgSignature(params: {
  token: string;
  timestamp: string;
  nonce: string;
  encrypt: string;
 ...
 * 验证企业微信签名
 */
export function verifyWecomKfSignature(params: {
  token: string;
  timestamp: string;
  nonce: string;
  encrypt: string;
  ...
  const expected = computeWecomKfMsgSignature({
    token: params.token,
 ...
 * 解密企业微信消息
 */
export function decryptWecomKfEncrypted(params: {
 ...
 * 加密企业微信回复消息
 */
export function encryptWecomKfPlaintext(params: {
  encodingAESKey: string;
  receiveId?: string;
  plaintext: string;
}): string {
