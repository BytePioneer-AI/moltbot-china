...
function resolveOpenKfId(account: ResolvedWecomKfAccount, override?: string): string | undefined {
  const candidate = override?.trim() || account.agentId?.trim();
...
export async function syncMessages(
  ...
  if (!account.corpId || !account.corpSecret || !openKfId) {
    return { errcode: -1, errmsg: "Account not configured for sync_msg (missing corpId, corpSecret, or agentId)" };
  }
...
export async function sendKfMessage(
  ...
  if (!account.corpId || !account.corpSecret || !openKfId) {
    return {
      ok: false,
      errcode: -1,
      errmsg: "Account not configured for sending (missing corpId, corpSecret, or agentId)",
    };
  }
