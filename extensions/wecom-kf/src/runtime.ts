...
export function getWecomKfRuntime(): PluginRuntime {
  if (!runtime) {
    throw new Error("WeCom KF runtime not initialized. Make sure the plugin is properly registered.");
  }
  return runtime;
}

/**
 * 尝试获取微信客服运行时（可能为 null）
 */
 ...
 * 检查运行时是否已初始化
 */
export function isWecomKfRuntimeInitialized(): boolean {
  return runtime !== null;
}
