...
import type { ResolvedWecomKhAccount, WecomKhInboundMessage, WecomKhDmPolicy } from "./types.js";
import {
  resolveAllowFrom,
  resolveDmPolicy,
  type PluginConfig,
...
export type WecomKhDispatchHooks = {
 ...
 * 提取消息内容
 */
export function extractWecomKhContent(msg: WecomKhInboundMessage): string {
...
function resolveSenderId(msg: WecomKhInboundMessage): string {
  return msg.external_userid?.trim() || "unknown";
}

function resolveChatId(_msg: WecomKhInboundMessage, senderId: string): string {
  return senderId;
}
 ...
 * 分发企业微信客户消息
 */
export async function dispatchWecomKhMessage(params: {
  cfg?: PluginConfig;
  account: ResolvedWecomKhAccount;
  msg: WecomKhInboundMessage;
  core: PluginRuntime;
  hooks: WecomKhDispatchHooks;
  log?: (msg: string) => void;
  ...
  const logger: Logger = createLogger("wecom-kh", { log: params.log, error: params.error });
  ...
  const rawBody = extractWecomKhContent(msg);
  ...
  const from = `wecom-kh:user:${senderId}`;
  ...
  if (channel.session?.recordInboundSession && storePath) {
    ...
    await channel.session.recordInboundSession({
      ...
      onRecordError: (err: unknown) => {
        logger.error(`wecom-kh: failed updating session meta: ${String(err)}`);
      },
    });
  }

  const tableMode = channel.text?.resolveMarkdownTableMode
    ? channel.text.resolveMarkdownTableMode({ cfg: safeCfg, channel: "wecom-kh", accountId: account.accountId })
    : undefined;
